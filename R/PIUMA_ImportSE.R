...
#'   \item  graph A list containing the igraph object derived from Jaccard matrix
#'   and intermediary objects
#'   \item  clustering A list containing two data.frames indicating the clustering
#'   per node and per rows
#'   }
#'
...
makeTDAobjFromSE <- function(SE, outcomes) {
  ...
  if (length(indFact) != 0) {
    ...
    df_fact_num <- as.data.frame(vapply(
      df_fact,
      as.numeric,
      numeric(nrow(df_fact))
    )) - 1
  ...
  PIUMA_tda_object <- new("TDAobj",
    ...
    outcome = df_new_out,
    comp = data.frame(),
    dist_mat = matrix(numeric(0), 0, 0),
    dfMapper = data.frame(),
    jacc = matrix(numeric(0), 0, 0),
