# Utility functions, for both graphs-related metrics, graph generation,miscellaneous
...
plot_ScaleFreeLaw <- function(dataPlDist, rCorkpk, rCorlogklogpk) {
  ...
  p2 <- p1 + scale_x_log10(
    ...
    annotation_logticks() +
    ggtitle(paste0(
      "Degree distr. (log-log scale) --- R = ",
      round(rCorlogklogpk[["r"]][2], 2)
    )) +
    theme(plot.title = element_text(size = 10, face = "bold"))


  show(patchwork::wrap_plots(p1, p2))
}
  ...
  if (is.null(vectColor)) {
    show(
      ggplot(allCmp, aes(x = comp1, y = comp2)) +
        geom_point(
          size = 2, alpha = 0.5, fill = "blue",
          color = "black", shape = 21
        ) +
        theme_bw(base_size = 10) +
        labs(
          title = method, x = "Component 1", y = "Component 2",
      ...
      ggplot(allCmp, aes(x = comp1, y = comp2, fill = vectColor)) +
        ...
        ) +
        {
          if (is.factor(vectColor)) {
            myCols <- c(
              "springgreen1", "red", "yellow", "white", "cyan",
